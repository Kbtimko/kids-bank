...
export async function POST(req: NextRequest) {
  ...
  const multiplier = parseFloat((await getSetting("interest_multiplier", family.familyId)) ?? "2");
  const floor = parseFloat((await getSetting("interest_floor_percent", family.familyId)) ?? "5");
  const { rate: fedRate } = await getFedRate();
  const effectiveRate = overrideRate ?? computeEffectiveRate(fedRate, multiplier, floor);

  const children = await sql`SELECT id, name FROM children WHERE family_id = ${family.familyId} ORDER BY id`;
  ...
  for (const child of children.rows as { id: number; name: string }[]) {
    const existing = await sql`
      ...
      WHERE child_id = ${child.id} AND type = 'interest' AND transaction_date >= ${monthStart}
    `;
  ...
  if (!body.confirm) {
    return NextResponse.json({ preview: previews, effectiveRate, fedRate, multiplier, floor, month: monthLabel });
  }
